...
fn test_fused_add_rmsnorm_determinism() {
    use crate::ops::cpu_fused_add_rmsnorm;
    use prelude_core::tensor::{DType, Device, Tensor};
...
fn test_layer_chain_determinism() {
    ...
    let s1: Vec<u16> = crate::ops::tensor_as_u16_slice_pub(&h1).unwrap().to_vec();
    let s2: Vec<u16> = crate::ops::tensor_as_u16_slice_pub(&h2).unwrap().to_vec();
    ...
    let ns1: Vec<u16> = crate::ops::tensor_as_u16_slice_pub(&no1).unwrap().to_vec();
    let ns2: Vec<u16> = crate::ops::tensor_as_u16_slice_pub(&no2).unwrap().to_vec();
...
fn test_linear_chain_determinism() {
    ...
    let ps1: Vec<u16> = crate::ops::tensor_as_u16_slice_pub(&p1).unwrap().to_vec();
    let ps2: Vec<u16> = crate::ops::tensor_as_u16_slice_pub(&p2).unwrap().to_vec();
    ...
    let fs1: Vec<u16> = crate::ops::tensor_as_u16_slice_pub(&f1).unwrap().to_vec();
    let fs2: Vec<u16> = crate::ops::tensor_as_u16_slice_pub(&f2).unwrap().to_vec();
...
fn test_multi_layer_determinism() {
    ...
    fn run_chain(
        ...
        for (i, lw) in layers.iter().enumerate() {
            ...
            let h2_raw: Vec<u16> = crate::ops::tensor_as_u16_slice_pub(&h2).unwrap().to_vec();
        ...
        crate::ops::tensor_as_u16_slice_pub(&out).unwrap().to_vec()
    }
