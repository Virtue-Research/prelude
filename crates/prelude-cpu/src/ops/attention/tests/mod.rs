...
fn naive_attention(q: &[f32], ks: &[Vec<f32>], vs: &[Vec<f32>], sm_scale: f32) -> Vec<f32> {
    let head_dim = q.len();
    let seq_len = ks.len();
...
fn gen_test_data(
    ...
    for i in 0..q.len() {
        q[i] = to_bf16(((i as f32 * 0.007) - 0.5).sin());
    }
    for i in 0..k.len() {
        k[i] = to_bf16(((i as f32 * 0.013) + 0.2).cos());
    }
    for i in 0..v.len() {
        v[i] = to_bf16(((i as f32 * 0.017) - 0.3).sin());
    }
    (q, k, v)
}
