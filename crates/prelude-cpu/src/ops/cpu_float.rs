...
pub trait CpuFloat: Copy + Send + Sync + 'static {
    ...
    /// (Copy is required because candle's storage guard is temporary.)
...
impl CpuFloat for half::bf16 {
    ...
    fn tensor_to_vec(tensor: &Tensor) -> Result<Vec<Self>> {
        ...
        let data = match &*storage {
            ...
            _ => prelude_core::tensor::bail!("CpuFloat::tensor_to_vec: not a CPU tensor"),
        };
...
impl CpuFloat for half::f16 {
    ...
    fn tensor_to_vec(tensor: &Tensor) -> Result<Vec<Self>> {
        let (storage, layout) = tensor.storage_and_layout();
        let data = match &*storage {
            prelude_core::tensor::backend::Storage::Cpu(s) => s.as_slice::<half::f16>()?,
            _ => prelude_core::tensor::bail!("CpuFloat::tensor_to_vec: not a CPU tensor"),
        };
        Ok(data[layout.start_offset()..].to_vec())
    }
