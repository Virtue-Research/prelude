...
use super::{MOD_KNORM_ROPE_KV_WRITE, MOD_KV_APPEND, PTX_KNORM_ROPE_KV_WRITE, PTX_KV_APPEND};
use candle_core::cuda_backend::cudarc::driver::{LaunchConfig, PushKernelArg};
use candle_core::cuda_backend::WrapErr;
use candle_core::{DType, Result, Tensor};

// ── Graph-safe KV cache append ───────────────────────────────────────
/// Append a single K/V token to the pre-allocated KV cache buffers.
///
/// This is CUDA-graph-safe: the write position is read from a device pointer,
/// so the graph can be replayed with different positions by updating the pointer
/// between replays.
///
/// - `k_buf`, `v_buf`: pre-allocated `(max_seq_len, num_kv_heads, head_dim)` BF16
/// - `k_new`, `v_new`: new token `(1, num_kv_heads, head_dim)` BF16 (or `(num_kv_heads, head_dim)`)
/// - `write_pos`: `[1]` U32 on device -- the position to write at
/// - `cu_seqlens_k`: `[2]` U32 on device -- `cu_seqlens_k[1]` is set to `write_pos + 1`
#[cfg(feature = "cuda")]
pub fn kv_cache_append_graphsafe(
    k_buf: &Tensor,
    v_buf: &Tensor,
    k_new: &Tensor,
    v_new: &Tensor,
    ...
    let dev = match k_buf.device() {
        candle_core::Device::Cuda(d) => d,
        _ => candle_core::bail!("kv_cache_append_graphsafe: not CUDA"),
    };

    // Extract CudaSlice references for each tensor
    let (k_buf_storage, k_buf_layout) = k_buf.storage_and_layout();
    let k_buf_cuda = match &*k_buf_storage {
        candle_core::Storage::Cuda(s) => s,
        _ => candle_core::bail!("k_buf not cuda"),
    };
    let k_buf_slice = k_buf_cuda.as_cuda_slice::<half::bf16>()?;
    let k_buf_slice = k_buf_slice.slice(k_buf_layout.start_offset()..);

    let (v_buf_storage, v_buf_layout) = v_buf.storage_and_layout();
    let v_buf_cuda = match &*v_buf_storage {
        candle_core::Storage::Cuda(s) => s,
        _ => candle_core::bail!("v_buf not cuda"),
    };
    let v_buf_slice = v_buf_cuda.as_cuda_slice::<half::bf16>()?;
    let v_buf_slice = v_buf_slice.slice(v_buf_layout.start_offset()..);

    let (k_new_storage, k_new_layout) = k_new.storage_and_layout();
    let k_new_cuda = match &*k_new_storage {
        candle_core::Storage::Cuda(s) => s,
        _ => candle_core::bail!("k_new not cuda"),
    };
    let k_new_slice = k_new_cuda.as_cuda_slice::<half::bf16>()?;
    let k_new_slice = k_new_slice.slice(k_new_layout.start_offset()..);

    let (v_new_storage, v_new_layout) = v_new.storage_and_layout();
    let v_new_cuda = match &*v_new_storage {
        candle_core::Storage::Cuda(s) => s,
        _ => candle_core::bail!("v_new not cuda"),
    };
    let v_new_slice = v_new_cuda.as_cuda_slice::<half::bf16>()?;
    let v_new_slice = v_new_slice.slice(v_new_layout.start_offset()..);

    let (wp_storage, wp_layout) = write_pos.storage_and_layout();
    let wp_cuda = match &*wp_storage {
        candle_core::Storage::Cuda(s) => s,
        _ => candle_core::bail!("write_pos not cuda"),
    };
    let wp_slice = wp_cuda.as_cuda_slice::<u32>()?;
    let wp_slice = wp_slice.slice(wp_layout.start_offset()..);

    let (cs_storage, cs_layout) = cu_seqlens_k.storage_and_layout();
    let cs_cuda = match &*cs_storage {
        candle_core::Storage::Cuda(s) => s,
        _ => candle_core::bail!("cu_seqlens_k not cuda"),
    };
    let cs_slice = cs_cuda.as_cuda_slice::<u32>()?;
    let cs_slice = cs_slice.slice(cs_layout.start_offset()..);

    let stride = (num_kv_heads * head_dim) as u32;
    let block_size = 256u32;
    let grid_size = (stride + block_size - 1) / block_size;

    let func = dev.get_or_load_custom_func("kv_cache_append_bf16", MOD_KV_APPEND, PTX_KV_APPEND)?;
    ...
    builder.arg(&k_buf_slice);
    builder.arg(&v_buf_slice);
    builder.arg(&k_new_slice);
    builder.arg(&v_new_slice);
    builder.arg(&wp_slice);
    builder.arg(&cs_slice);
    builder.arg(&stride);
    unsafe { builder.launch(cfg) }
        .map_err(|e| candle_core::Error::Msg(format!("kv_cache_append_bf16 launch: {e}")))?;
