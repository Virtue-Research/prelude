...
mod tests {
    ...
    fn make_test_q2k_blocks(values: &[f32]) -> Vec<BlockQ2K> {
        ...
        let qt = QTensor::quantize_onto(&t, GgmlDType::Q2K, &Device::Cpu).unwrap();
    ...
    fn scalar_vs_candle_dequant() {
        ...
        // Q2_K has higher error due to extreme compression
        let rel_err = (our_dot - ref_dot).abs() / ref_dot.abs().max(1e-6);
        assert!(rel_err < 0.15, "scalar vs dequant: our={our_dot}, ref={ref_dot}, rel_err={rel_err}");
    }
