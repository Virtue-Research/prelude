...
impl SimpleBackend for crate::tensor::safetensors::MmapedSafetensors {
    fn get(
        ...
        let tensor = Tensor::from_candle(self.load(name, dev)?.to_dtype(candle_core::DType::from(dtype))?);
        if tensor.shape() != &s {
            Err(crate::tensor::Error::UnexpectedShape {
                msg: format!("shape mismatch for {name}"),
                expected: s,
    ...
    fn get_unchecked(&self, name: &str, dtype: DType, dev: &Device) -> Result<Tensor> {
        Ok(Tensor::from_candle(self.load(name, dev)?.to_dtype(candle_core::DType::from(dtype))?))
    }
