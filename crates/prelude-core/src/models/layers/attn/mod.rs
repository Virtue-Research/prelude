...
//!
//! Backend priority: FA4 (CuTeDSL) → FA3 (Hopper) → FA2 (Ampere+) → CPU.
//!
//! Adding a new backend (e.g. FlashInfer):
//! 1. Create `flashinfer.rs` implementing `varlen_causal` etc.
//! 2. Add a dispatch branch below.
//! 3. Done — no model/engine/cache changes needed.
...
pub(crate) fn varlen_attention(
    ...
    if let Some(kv) = paged_kv {
        // FA3: flash layout cache + fused paged attention
        #[cfg(feature = "flash-attn-v3")]
        if q.device().is_cuda() {
...
///
/// Selects flash layout (FA3) or v1 layout (FA2/others) based on compiled features.
...
pub(crate) fn reshape_and_cache(
    ...
    #[cfg(all(feature = "cuda", not(feature = "flash-attn-v3")))]
...
fn cu_seqlens_to_lens(cu_seqlens: &Tensor) -> Result<Tensor> {
    ...
    let lens: Vec<u32> = cu.windows(2).map(|w| w[1] - w[0]).collect();
    Tensor::from_vec(lens, (cu.len() - 1,), cu_seqlens.device())
}
