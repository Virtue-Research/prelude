...
use super::linear::{wrap_linear, QwenLinear};
...
pub(crate) struct Qwen3Mlp {
    ...
    gate_up_proj: Option<QwenLinear>,
}

impl Qwen3Mlp {
    pub(crate) fn new(cfg: &Qwen3Config, vb: VarBuilder) -> Result<Self> {
        let gate_proj = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            cfg.intermediate_size,
            vb.pp("gate_proj"),
        )?)?;
        let up_proj = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            cfg.intermediate_size,
            vb.pp("up_proj"),
        )?)?;
        let down_proj = wrap_linear(candle_nn::linear_no_bias(
            cfg.intermediate_size,
            cfg.hidden_size,
            vb.pp("down_proj"),
        )?)?;
