...
// ── RMSNorm with +1 weight adjustment (Gemma specific) ───────────────────

/// Gemma3-specific RMSNorm that:
/// 1. Adds +1 to loaded weights (Gemma-specific)
/// 2. Uses fast_rmsnorm CUDA kernel on GPU
/// 3. Falls back to candle_nn::RmsNorm on CPU
#[derive(Debug, Clone)]
...
impl Gemma3RmsNorm {
    fn new(weight: Tensor, eps: f64) -> Self {
        Self {
            candle_norm: RmsNorm::new(weight.clone(), eps),
            weight,
            eps,
        }
    }
...
impl Module for Gemma3RmsNorm {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        if x.device().is_cuda() && x.dtype() == DType::BF16 {
            return crate::ops::gpu::fast_rmsnorm(x, &self.weight, self.eps);
        }
        self.candle_norm.forward(x)
    }
}
    ...
    fn new(
        ...
        Ok(Self {
            ...
            q_norm: {
                ...
                Gemma3RmsNorm::new(weight, cfg.rms_norm_eps)
            },
            k_norm: {
...
impl Gemma3DecoderLayer {
    fn new(
        ...
        Ok(Self {
            self_attn,
            mlp,
            input_layernorm: Gemma3RmsNorm::new(input_ln_adjusted, cfg.rms_norm_eps),
            post_attention_layernorm: Gemma3RmsNorm::new(post_attn_ln_adjusted, cfg.rms_norm_eps),
            pre_feedforward_layernorm: Gemma3RmsNorm::new(pre_ffn_ln_adjusted, cfg.rms_norm_eps),
            post_feedforward_layernorm: Gemma3RmsNorm::new(post_ffn_ln_adjusted, cfg.rms_norm_eps),
        })
    }
    ...
    fn forward(
        ...
        let xs = crate::models::layers::fast_add(&post_attn_normed, xs)?;
        ...
        crate::models::layers::fast_add(&post_ffn_normed, &xs)
    }
...
impl Gemma3Model {
    fn new(cfg: &Gemma3Config, vb: VarBuilder) -> Result<Self> {
        ...
        let norm = Gemma3RmsNorm::new(norm_weight.clone(), cfg.rms_norm_eps);
...
impl Gemma3ForCausalLM {
    ...
    pub fn forward(
        ...
        let last_hidden =
            crate::models::layers::last_token_select(&hidden, ctx.seq_lens)?.contiguous()?;
...
impl Gemma3ForSequenceClassification {
    ...
    pub fn new_with_parts(
        ...
        let score =
            candle_nn::linear_no_bias(cfg.base.hidden_size, cfg.num_labels, head_vb.pp("score"))?;
    ...
    pub fn forward(
        ...
        let last_hidden = crate::models::layers::last_token_select(&hidden_states, ctx.seq_lens)?;
    ...
    pub fn forward(
        ...
        let mut pooled = match self.pooling {
            crate::engine::EmbeddingPooling::LastToken => {
                crate::models::layers::last_token_select(&hidden_states, ctx.seq_lens)?
            }
            crate::engine::EmbeddingPooling::Mean => {
                pool_mean_varlen(&hidden_states, ctx.seq_lens)?
            }
            crate::engine::EmbeddingPooling::Cls => {
                crate::models::layers::first_token_select(&hidden_states, ctx.seq_lens)?
            }
        };
