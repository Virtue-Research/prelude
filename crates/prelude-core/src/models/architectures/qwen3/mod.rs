...
use crate::models::layers::{BatchAttnContext, LayerAttnContext};
use crate::models::layers::{
    Qwen3Mlp, Qwen3RotaryEmbedding, QwenLinear, QwenRmsNorm, fast_add, fast_rms_norm,
    fused_add_rmsnorm, last_token_select, qknorm_rope_varlen, wrap_linear, wrap_rmsnorm,
};
#[cfg(feature = "cuda")]
use crate::models::layers::debug_disable_fused_qknorm_rope;

// Re-export public debug setters so existing callers (`use qwen3::set_debug_*`) still compile.
pub use crate::models::layers::{
...
impl Qwen3Attention {
    pub(crate) fn new(
        ...
        let q_norm_rms = candle_nn::RmsNorm::new(q_norm_weight_raw.clone(), cfg.rms_norm_eps);
        let k_norm_vb = vb.pp("k_norm");
        let k_norm_rms = candle_nn::rms_norm(head_dim, cfg.rms_norm_eps, k_norm_vb.clone())?;
        let k_norm_weight = k_norm_vb.get(head_dim, "weight")?;
        let is_cuda = vb.device().is_cuda();

        let q_proj = wrap_linear(candle_nn::linear_b(
            cfg.hidden_size,
            num_heads * head_dim,
            cfg.attention_bias,
            vb.pp("q_proj"),
        )?)?;
        let k_proj = wrap_linear(candle_nn::linear_b(
            cfg.hidden_size,
            num_kv_heads * head_dim,
            cfg.attention_bias,
            vb.pp("k_proj"),
        )?)?;
        let v_proj = wrap_linear(candle_nn::linear_b(
            cfg.hidden_size,
            num_kv_heads * head_dim,
            cfg.attention_bias,
            vb.pp("v_proj"),
        ...
        let qkv_proj = {
            ...
            if qw.device().is_cpu() && qw.dtype() == DType::BF16 {
                ...
                match wrap_linear(candle_nn::Linear::new(merged_w, None)) {
                    Ok(l) => Some(l),
                    Err(e) => {
        ...
        Ok(Self {
            ...
            k_proj,
            v_proj,
            o_proj: wrap_linear(candle_nn::linear_b(
                num_heads * head_dim,
                cfg.hidden_size,
                cfg.attention_bias,
                vb.pp("o_proj"),
            )?)?,
            q_norm: wrap_rmsnorm(q_norm_rms, cfg.rms_norm_eps, q_norm_weight_raw.clone()),
            k_norm: wrap_rmsnorm(k_norm_rms, cfg.rms_norm_eps, k_norm_weight.clone()),
    ...
    #[inline]
    fn prepare_qkv_varlen(&self, x: &Tensor, total: usize) -> Result<(Tensor, Tensor, Tensor)> {
        crate::models::layers::attention::prepare_qkv_varlen(
            x,
            ...
            if self.is_cuda && !debug_disable_fused_qknorm_rope() {
                ...
                if let Some(kv) = ctx.paged_kv {
                    ...
                    if !used_fused_kv_write {
                        ...
                        crate::models::layers::reshape_and_cache(
                            &k,
                            &v,
                            kv.key_cache,
                            kv.value_cache,
                    ...
                    let attn = crate::models::layers::varlen_attention_paged(
                        &q,
                        kv.key_cache,
                        kv.value_cache,
                        kv.block_tables,
    ...
    fn forward_with_cache(&mut self, x: &Tensor, position_offset: usize) -> Result<Tensor> {
        ...
        let (q, k, v) = self.prepare_qkv_varlen(x, seq_len)?;
    ...
    fn matmul_cross_attention_candle(
        ...
        let (k, v) = if gqa_ratio > 1 {
            (
                k.reshape((1, self.num_kv_heads, 1, total_kv_len, self.head_dim))?
                    .expand((1, self.num_kv_heads, gqa_ratio, total_kv_len, self.head_dim))?
                    .reshape((1, self.num_heads, total_kv_len, self.head_dim))?
                    .contiguous()?,
                v.reshape((1, self.num_kv_heads, 1, total_kv_len, self.head_dim))?
                    .expand((1, self.num_kv_heads, gqa_ratio, total_kv_len, self.head_dim))?
                    .reshape((1, self.num_heads, total_kv_len, self.head_dim))?
                    .contiguous()?,
            )
        ...
        let scores = if seq_len > 1 {
            ...
            for i in 0..seq_len {
                for j in (position_offset + i + 1)..total_kv_len {
                    mask_data[i * total_kv_len + j] = f32::NEG_INFINITY;
                }
            }
            let mask =
                Tensor::from_vec(mask_data, (1, 1, seq_len, total_kv_len), q.device())?;
...
impl Qwen3ForSequenceClassification {
    pub fn new(cfg: &Qwen3ClassifierConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let score = wrap_linear(candle_nn::linear_no_bias(
