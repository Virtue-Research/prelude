...
use crate::models::layers::varlen_attention;

use crate::models::layers::{
    fast_rms_norm, last_token_select, wrap_linear, wrap_rmsnorm, BatchAttnContext,
    LayerAttnContext, QwenLinear, QwenRmsNorm,
};
...
struct Qwen3_5GatedDeltaNet {
    ...
    in_proj_qkv: QwenLinear, // hidden → key_dim*2 + value_dim
    in_proj_z: QwenLinear,   // hidden → value_dim
    in_proj_b: QwenLinear,   // hidden → num_v_heads
    in_proj_a: QwenLinear,   // hidden → num_v_heads
...
impl Qwen3_5GatedDeltaNet {
    fn new(cfg: &Qwen3_5Config, vb: VarBuilder) -> Result<Self> {
        ...
        let in_proj_qkv = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            key_dim * 2 + value_dim, // Q + K + V
            vb.pp("in_proj_qkv"),
        )?)?;
        let in_proj_z = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            value_dim,
            vb.pp("in_proj_z"),
        )?)?;
        let in_proj_b = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            cfg.linear_num_value_heads,
            vb.pp("in_proj_b"),
        )?)?;
        let in_proj_a = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            cfg.linear_num_value_heads,
            vb.pp("in_proj_a"),
        )?)?;
        ...
        let out_proj = wrap_linear(candle_nn::linear_no_bias(
            value_dim,
            cfg.hidden_size,
            vb.pp("out_proj"),
        )?)?;
...
impl Qwen3_5Attention {
    fn new(cfg: &Qwen3_5Config, rope: PartialRotaryEmbedding, vb: VarBuilder) -> Result<Self> {
        ...
        let q_proj = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            q_proj_dim,
            vb.pp("q_proj"),
        )?)?;
        let k_proj = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            kv_proj_dim,
            vb.pp("k_proj"),
        )?)?;
        let v_proj = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            kv_proj_dim,
            vb.pp("v_proj"),
        )?)?;
        let o_proj = wrap_linear(candle_nn::linear_no_bias(
            cfg.num_attention_heads * cfg.head_dim,
            cfg.hidden_size,
            vb.pp("o_proj"),
        )?)?;
        ...
        let q_norm_rms = RmsNorm::new(q_norm_weight.clone(), cfg.rms_norm_eps);
        let q_norm = wrap_rmsnorm(q_norm_rms, cfg.rms_norm_eps, q_norm_weight.clone());

        let k_norm_weight = (vb.pp("k_norm").get(cfg.head_dim, "weight")? + 1.0)?;
        let k_norm_rms = RmsNorm::new(k_norm_weight.clone(), cfg.rms_norm_eps);
        let k_norm = wrap_rmsnorm(k_norm_rms, cfg.rms_norm_eps, k_norm_weight.clone());
...
struct Qwen3_5SparseMoeBlock {
    gate: Linear, // [num_experts, hidden_size]
    ...
    shared_expert_gate: Option<QwenLinear>, // [1, hidden_size]
...
impl Qwen3_5SparseMoeBlock {
    fn new(cfg: &Qwen3_5Config, vb: VarBuilder) -> Result<Self> {
        ...
        let shared_expert_gate = if shared_expert.is_some() {
            Some(wrap_linear(candle_nn::linear_no_bias(
                cfg.hidden_size,
                1,
...
impl Qwen3_5DecoderLayer {
    fn new(
        ...
        let input_layernorm = wrap_rmsnorm(
            RmsNorm::new(input_ln_weight.clone(), cfg.rms_norm_eps),
            cfg.rms_norm_eps,
            input_ln_weight.clone(),
        );
        let post_attn_ln_weight = (vb
            .pp("post_attention_layernorm")
            .get(cfg.hidden_size, "weight")?
            + 1.0)?;
        let post_attention_layernorm = wrap_rmsnorm(
            RmsNorm::new(post_attn_ln_weight.clone(), cfg.rms_norm_eps),
            cfg.rms_norm_eps,
            post_attn_ln_weight.clone(),
        );
...
impl Qwen3_5Model {
    fn new(cfg: &Qwen3_5Config, vb: VarBuilder) -> Result<Self> {
        ...
        let norm = wrap_rmsnorm(
            RmsNorm::new(norm_weight.clone(), cfg.rms_norm_eps),
            cfg.rms_norm_eps,
            norm_weight.clone(),
        );
