...
struct Qwen3NextGatedDeltaNet {
    in_proj_qkvz: QwenLinear,
    in_proj_ba: QwenLinear,
...
impl Qwen3NextGatedDeltaNet {
    fn new(cfg: &Qwen3NextConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let in_proj_qkvz = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            proj_dim,
            vb.pp("in_proj_qkvz"),
        )?)?;
        let in_proj_ba = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
...
struct Qwen3NextAttention {
    q_proj: QwenLinear, // output is 2x width: Q + gate
    k_proj: QwenLinear,
    v_proj: QwenLinear,
    o_proj: QwenLinear,
    q_norm: QwenRmsNorm,
...
impl Qwen3NextSparseMoeBlock {
    fn new(cfg: &Qwen3NextConfig, vb: VarBuilder) -> Result<Self> {
        let gate = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            cfg.num_experts,
            vb.pp("gate"),
        )?)?;
        ...
        let shared_expert_gate = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            1,
...
impl Qwen3NextForCausalLM {
    pub fn new(cfg: &Qwen3NextConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let lm_head = wrap_linear(candle_nn::linear_no_bias(
            cfg.hidden_size,
            cfg.vocab_size,
            vb.pp("lm_head"),
        )?)?;
