...
fn main() -> Result<()> {
    ...
    // Our final library name. Renamed from upstream's `libflashattentionv3.a`
    // to avoid colliding with the workspace fork's identically-named archive
    // (and any CANDLE_FLASH_ATTN_BUILD_DIR cache) — see fa3-prebuilt-kernel-cache.
    let out_file = build_dir.join("libcandle_fa3_0102.a");
    builder.build_lib(out_file)?;

    // Finally, instruct cargo to link your library
    println!("cargo:rustc-link-search={}", build_dir.display());
    println!("cargo:rustc-link-lib=static=candle_fa3_0102");

    // Newer FA3 (vendored vllm-flash-attn hopper) varlen kernel, prebuilt out-of-band by
    // _build_v3_kernel.sh (CUDA 12.9 + CUTLASS 3.8.0) into FA3_V3_PREBUILT_DIR. Linked
    // alongside the 0.10.2 kernel; provides ffi::run_mha_v3 (flash_attn_varlen_v3).
    let v3_dir = std::env::var("FA3_V3_PREBUILT_DIR")
        .unwrap_or_else(|_| "/data/xueying/fa3_v3_prebuilt".to_string());
    if std::path::Path::new(&format!("{v3_dir}/libcandle_fa3_v3.a")).exists() {
        println!("cargo:rustc-link-search={v3_dir}");
        println!("cargo:rustc-link-lib=static=candle_fa3_v3");
        println!("cargo:rustc-cfg=fa3_v3");
    }
