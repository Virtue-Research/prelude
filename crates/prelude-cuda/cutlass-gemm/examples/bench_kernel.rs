...
fn rand_fp16(len: usize) -> Vec<half::f16> {
    ...
    let mut rng = rand::rng();
    (0..len)
        .map(|_| half::f16::from_f32(rng.random_range(-0.5f32..0.5f32)))
        .collect()
}
...
fn print_header(title: &str) {
    ...
    print!(
        "{:<10} {:<8} {:>9} {:>9} {:>7} {:>7}",
        "tokens", "layer", "SM90", "cuBLAS", "vs_cub", "TFLOPS"
    );
    println!();
    println!("{}", "-".repeat(80));
}
        ...
        for (m, tok_label) in &token_counts() {
            ...
            for (layer, n, k) in &layers {
                ...
                let sm90_us = bench_us(
                    || cutlass_gemm_bf16(&weight, &input, &mut out, m, *n, *k, &gpu),
                    &gpu,
                );
                ...
                for cfg in 0..3i32 {
                    let us = bench_us(
                        || cutlass_sm80_bf16(&weight, &input, &mut out, m, *n, *k, cfg, &gpu),
                        &gpu,
                    );
                    sm80_times.push(us);
                }
    ...
    for (name, h, i, _v) in &models() {
        ...
        for (m, tok_label) in &token_counts() {
            ...
            for (layer, n, k) in &layers {
                ...
                let cub_us = bench_us(
                    || cublas_gemm_fp16(&weight, &input, &mut out, m, *n, *k, &gpu),
                    &gpu,
                );
                ...
                let r = cub_us / sm90_us;
                println!(
                    "{tok_label:<10} {layer:<8} {sm90_us:>9.1} {cub_us:>9.1} {:>5.2}x{:<1} {tf:>6.1}T",
                    r,
                    speedup_marker(r)
                );
            }
    ...
    for (name, h, i, _v) in &models() {
        ...
        for (m, tok_label) in &token_counts() {
            ...
            for (layer, n, k) in &layers {
                ...
                let cub_us = bench_us(
                    || cublas_gemm_f32(&weight, &input, &mut out, m, *n, *k, &gpu),
                    &gpu,
                );
    ...
    for (name, h, i, _v) in &models() {
        ...
        for (m, tok_label) in &token_counts() {
            ...
            for (layer, n, k) in &layers {
                ...
                let fp8_data: Vec<u8> = a_f32
                    .iter()
                    .map(|&x| float8::F8E4M3::from_f32(x * scale_b).to_bits())
                    .collect();
                let fp8_weight: Vec<u8> = b_f32
                    .iter()
                    .map(|&x| float8::F8E4M3::from_f32(x * scale_a).to_bits())
                    .collect();
                ...
                if cub_ok {
                    ...
                    println!(
                        "{tok_label:<10} {layer:<8} {sm90_us:>9.1}       N/A         {tf:>6.1}T"
                    );
                }
            }
        }
...
fn cublas_batched_bf16(
    ...
    unsafe {
        gpu.blas
            .gemm_strided_batched(cfg, weight, input, output)
            .unwrap();
    }
}
    ...
    for (batch, m, n, k) in &shapes {
        ...
        let sm90_us = bench_us(
            || {
                ...
                let (op, _g3) = out.device_ptr_mut(&gpu.stream);
                cutlass_dispatch(
                    wp as _, ip as _, op as _, *m, *n, *k, *batch, stride_a, stride_b, stride_d, 0,
                    &gpu,
                );
            },
            &gpu,
        ...
        let r = cub_us / sm90_us;
        println!(
            "{batch:<8} {m:<6} {n:<6} {k:<6} {sm90_us:>9.1} {cub_us:>9.1} {:>5.2}x{:<1} {tf:>6.1}T",
            r,
            speedup_marker(r)
        );
    }
