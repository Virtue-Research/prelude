...
pub fn tiled_mmq(
    ...
    if a_cuda.dtype() != DType::BF16 {
        bail!(
            "tiled_mmq: activations must be BF16, got {:?}",
            a_cuda.dtype()
        );
    }
    ...
    // Get raw CUDA stream
    let raw_stream = stream.cu_stream() as *const c_void;
    ...
    unsafe {
        quant_gemm::quantize_q8_1(a_ptr, q8_ptr, m as i64, k as i64, weight_type, raw_stream);
    }
