...
extern "C" __global__ void fast_rmsnorm_bf16_f32_weight(
    const __nv_bfloat16* __restrict__ input,   // [N, D]
    ...
    if (d <= 256) {
        ...
        for (uint32_t e = 0; e < elems_per_lane; e++) {
            uint32_t idx = lane_id * elems_per_lane + e;
            out_row[idx] = __float2bfloat16(vals[e] * scale * weight[idx]);
        }
        return;
    }
    ...
    if (use_cached_vec) {
        ...
        for (int j = 0; j < 4; j++) {
            float2 r;
            r.x = vals[j * 2]     * scale * weight[tid * 8 + j * 2];
            r.y = vals[j * 2 + 1] * scale * weight[tid * 8 + j * 2 + 1];
            op[j] = __float22bfloat162_rn(r);
        }
        result.store(out_row + tid * 8);
    } else {
        for (uint32_t i = tid; i < d; i += block_size) {
            float v = __bfloat162float(in_row[i]);
            out_row[i] = __float2bfloat16(v * scale * weight[i]);
        }
    }
}
