...
struct CollectiveMainloopFwdSm90 {
    ...
    struct Arguments {
        ...
        // Prelude Q-prologue fusion: fused RMSNorm(+existing rotary) on Q.
        Element const* const ptr_qnorm_weight = nullptr;
        float const qnorm_eps = 1e-6f;
    };
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.ptr_Q, args.shape_Q, args.stride_Q, shape_Q_packed, stride_Q_packed,
                ...
                args.ptr_qnorm_weight, args.qnorm_eps};
    }
    ...
    mma(Params const& params,
        ...
        auto &barrier_Q = shared_storage.pipelines.barrier_Q;
        if constexpr (!AppendKV) {
            if (params.ptr_qnorm_weight != nullptr && get<1>(params.shape_rotary) > 0) {
                // Prelude Q-prologue fusion: per-head RMSNorm + RoPE applied to
                // the RAW Q tile in SMEM (the standalone qknorm_rope pass and
                // its HBM round-trip are eliminated). Positions are continuous
                // per sequence: start = seqlen_k - seqlen_q — the paged KV
                // cache already contains the current chunk, so Q row i sits at
                // absolute position seqlen_k - seqlen_q + i (matches the
                // engine's position_ids for prefill chunks and decode alike).
                using Rotary_t = Rotary<kBlockM, kHeadDim, NumMmaThreadsQK, Element, false /*FixedPosition*/>;
                int const start_idx = seqlen_info.seqlen_k - seqlen_info.seqlen_q;
                Rotary_t rotary(params.ptr_rotary_cos, params.shape_rotary, params.stride_rotary_cos,
                                ...
                                false /*is_rotary_interleaved*/, thread_idx, seqlen_q, start_idx);
                Tensor sQ_pi = cute::as_position_independent_swizzle_tensor(sQ);
                int const qhead_per_khead = !PackGQA ? 1 : params.qhead_per_khead_divmod.divisor;
                // gmem cos/sin loads overlap with the Q TMA (before barrier_Q).
                ...
                rotary.apply_Q_norm_rope_contiguous(sQ_pi, tRrCosCont, tRrSinCont,
                                                    params.ptr_qnorm_weight, params.qnorm_eps,
                                                    m_block, qhead_per_khead);
                // SMEM fence so the normalized+rotated Q is visible to GMMA
                cutlass::arch::fence_view_async_shared();
