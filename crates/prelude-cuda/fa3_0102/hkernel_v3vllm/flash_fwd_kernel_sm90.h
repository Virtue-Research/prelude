...
// FA3_CLOCK_PROBE: per-CTA timing probe globals (fa3_probe_*) are defined at file scope in
// the base instantiation .cu BEFORE this header is included, so operator() can use them.
...
public:
    ...
    operator()(Params const& params, char* smem_buf) {
        ...
        if (threadIdx.x == 0 && blockIdx.x < 256) {
            unsigned smid_; asm volatile("mov.u32 %0, %%smid;" : "=r"(smid_));
            fa3_probe_sm[blockIdx.x] = smid_;
            fa3_probe_start[blockIdx.x] = (unsigned long long)clock64();
            fa3_probe_end[blockIdx.x] = 0ull;
        }
#endif
...
#ifdef FA3_CLOCK_PROBE
        if (blockIdx.x < 256) {
            atomicMax((unsigned long long*)&fa3_probe_end[blockIdx.x], (unsigned long long)clock64());
        }
#endif
    }
