...
// FA3_CUBIN transplant hook (build with -DFA3_TRANSPLANT_HOOK; needs -lcuda): if env
// FA3_CUBIN points to a cubin, launch the kernel with the SAME mangled name from that cubin
// (driver API) instead of the statically-linked one. Used to A/B vLLM's shipped compiled
// artifact against ours under an identical host harness.
#ifdef FA3_TRANSPLANT_HOOK
#include <cuda.h>
#include <cstdlib>
#include <cstdio>
#include <unordered_map>
static inline bool fa3_transplant_launch(void const* func, dim3 grid, dim3 block, int smem,
                                         cudaStream_t stream, void* arg) {
    const char* path = std::getenv("FA3_CUBIN");
    if (!path) return false;
    static CUmodule mod = nullptr;
    static std::unordered_map<void const*, CUfunction> cache;
    if (!mod) {
        CUresult r = cuModuleLoad(&mod, path);
        if (r != CUDA_SUCCESS) { fprintf(stderr, "FA3_CUBIN: cuModuleLoad(%s) failed: %d\n", path, (int)r); exit(1); }
    }
    CUfunction f;
    auto it = cache.find(func);
    if (it == cache.end()) {
        const char* name = nullptr;
        if (cudaFuncGetName(&name, func) != cudaSuccess || !name) {
            fprintf(stderr, "FA3_CUBIN: cudaFuncGetName failed\n"); exit(1);
        }
        CUresult r = cuModuleGetFunction(&f, mod, name);
        if (r != CUDA_SUCCESS) { fprintf(stderr, "FA3_CUBIN: kernel %s not in %s (%d)\n", name, path, (int)r); exit(1); }
        // dynamic smem far exceeds the 48KB default; must opt in explicitly on the driver handle
        cuFuncSetAttribute(f, CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES, smem);
        size_t poff = 0, psz = 0;
        cuFuncGetParamInfo(f, 0, &poff, &psz);
        fprintf(stderr, "FA3_CUBIN: resolved kernel, cubin param0 size=%zu offset=%zu\n", psz, poff);
        cache.emplace(func, f);
    } else { f = it->second; }
    if (const char* dp = std::getenv("FA3_DUMP_PARAMS")) {
        static bool dumped = false;
        if (!dumped) { FILE* fp = fopen(dp, "wb"); if (fp) { fwrite(arg, 1, 2688, fp); fclose(fp); dumped = true; } }
    }
    void* args[] = { arg };
    CUresult r = cuLaunchKernel(f, grid.x, grid.y, grid.z, block.x, block.y, block.z,
                                (unsigned)smem, stream, args, nullptr);
    if (r != CUDA_SUCCESS) { fprintf(stderr, "FA3_CUBIN: cuLaunchKernel failed: %d\n", (int)r); exit(1); }
    return true;
}
#else
static inline bool fa3_transplant_launch(void const*, dim3, dim3, int, cudaStream_t, void*) { return false; }
#endif  // FA3_TRANSPLANT_HOOK
...
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    typename CollectiveMainloop::Arguments mainloop_args {
        ...
        static_cast<Element const*>(params.qnorm_weight_ptr), params.qnorm_eps
    };
    ...
    if constexpr (size(ClusterShape{}) > 1) {
        ...
        if (!fa3_transplant_launch((void const*)kernel, grid_dims, block_dims, smem_size, stream,
                                   (void*)&kernel_params)) {
