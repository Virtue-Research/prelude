...
struct Rotary {
    ...
    // Fused per-head RMSNorm + non-interleaved RoPE on Q in SMEM (prelude
    // Q-prologue fusion). Math identical to the standalone fused_qknorm_rope
    // kernel: fp32 sum-of-squares over the whole head row, scale = rsqrt(ss/D
    // + eps), x *= scale * weight[d], then rotate-half RoPE.
    //
    // DIVERGENCE-SAFETY (the fa3_q_prologue_deadlock_fix lesson): the row
    // sum-of-squares is reduced across the kGmemThreadsPerRow lanes sharing a
    // row via full-mask __shfl_xor_sync. Threads must NOT exit early before
    // the shuffles — out-of-range rows still participate (their SMEM reads are
    // in-bounds garbage, their group's reduction is self-contained and never
    // written back). Row validity is uniform within an 8-lane row group, and
    // `valid` only gates the final write-back.
    //
    // Requires rotary_dim == kHeadDim (full-rope models like Qwen3).
    template <typename TensorsQ, typename TensortRrR>
    CUTLASS_DEVICE
    void
    apply_Q_norm_rope_contiguous(TensorsQ &sQ,  // (kBlockM, kHeadDim)
                                 TensortRrR const &tRrCosCont, // (kBlockM, kHeadDim / 2)
                                 TensortRrR const &tRrSinCont, // (kBlockM, kHeadDim / 2)
                                 Element const* const qnorm_weight,  // [kHeadDim] gmem
                                 float const qnorm_eps,
                                 int const m_block, int const qhead_per_khead=1)
    {
        TiledCopyQK tiled_copy_q;
        auto gmem_thr_copy_q = tiled_copy_q.get_thread_slice(thread_idx);
        ...
        int const row_limit = std::min(max_seqlen * qhead_per_khead - m_block * kBlockMN, kBlockMN);
        // For hdim128 size<2>(tQcQ) == 1, so the fragments of the single k-chunk
        // stay live in registers across the reduction — single SMEM read pass.
        static_assert(decltype(size<2>(tQcQ))::value == 1,
                      "apply_Q_norm_rope_contiguous assumes one k-chunk per thread (hdim 128)");

        #pragma unroll
        for (int m = 0; m < size<1>(tQcQ); ++m) {
            int const row = get<0>(tQcQ(_0{}, m, _0{}));
            bool const valid = row < row_limit;
            int const col = get<1>(tQcQ(_0{}, _0{}, _0{}));
            int const col_idx_left = col / kGmemElemsPerLoad;
            int const col_idx_right = col / kGmemElemsPerLoad + kHeadDim / (2 * kGmemElemsPerLoad);

            // SMEM reads are always in-bounds (the sQ tile is fully allocated),
            // so every lane loads unconditionally — no divergence before the
            // warp-collective reduction below.
            Tensor rQ_left = make_fragment_like(sQ_copy(_, row, col_idx_left));
            cute::copy(tiled_copy_q, sQ_copy(_, row, col_idx_left), rQ_left);
            Tensor rQ_right = make_fragment_like(rQ_left);
            cute::copy(tiled_copy_q, sQ_copy(_, row, col_idx_right), rQ_right);

            float l_f[CUTE_STATIC_V(size<0>(rQ_left))];
            float r_f[CUTE_STATIC_V(size<0>(rQ_left))];
            float ss = 0.f;
            #pragma unroll
            for (int i = 0; i < size<0>(rQ_left); ++i) {
                l_f[i] = static_cast<float>(rQ_left[i]);
                r_f[i] = static_cast<float>(rQ_right[i]);
                ss += l_f[i] * l_f[i] + r_f[i] * r_f[i];
            }
            // ── Row reduction across the lanes sharing this row. ALL lanes
            // participate (full mask); xor offsets < kGmemThreadsPerRow stay
            // inside the lane-aligned row group (divergence-safety: `continue`
            // only after all warp-collectives).
            #pragma unroll
            for (int offset = kGmemThreadsPerRow / 2; offset > 0; offset >>= 1) {
                ss += __shfl_xor_sync(0xffffffff, ss, offset);
            }
            float const scale = rsqrtf(ss / float(kHeadDim) + qnorm_eps);

            if (!valid) { continue; }
            Tensor rCos = tRrCosCont(_, m, _0{});
            Tensor rSin = tRrSinCont(_, m, _0{});
            #pragma unroll
            for (int i = 0; i < size<0>(rQ_left); ++i) {
                float const wl = static_cast<float>(qnorm_weight[col + i]);
                float const wr = static_cast<float>(qnorm_weight[col + i + kHeadDim / 2]);
                float const l = l_f[i] * scale * wl;
                float const r = r_f[i] * scale * wr;
                float const c = static_cast<float>(rCos[i]);
                float const s = static_cast<float>(rSin[i]);
                rQ_left[i] = static_cast<Element>(l * c - r * s);
                rQ_right[i] = static_cast<Element>(l * s + r * c);
            }
            cute::copy(tiled_copy_q, rQ_left, sQ_copy(_, row, col_idx_left));
            cute::copy(tiled_copy_q, rQ_right, sQ_copy(_, row, col_idx_right));
        }
