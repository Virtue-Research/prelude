...
// SM100 functions (defined in deepgemm_sm100.cu, linked separately).
// Weak symbols: resolve to stubs if SM100 compilation unit is not linked.
extern "C" __attribute__((weak)) int deepgemm_sm100_bf16_gemm(
    void*, void*, void*, int, int, int, void*) { return -1; }
extern "C" __attribute__((weak)) void deepgemm_sm100_query_config(
...
int deepgemm_bf16_gemm(
    ...
    if (g_gpu_arch >= 100)
        return deepgemm_sm100_bf16_gemm(A, B, D, M, N, K, stream);
...
void deepgemm_query_config(int M, int N, int K,
    ...
    ensure_gpu_arch();
    if (g_gpu_arch >= 100) {
        deepgemm_sm100_query_config(M, N, K, out_block_m, out_block_n, out_stages, out_smem);
        return;
    }
    auto cfg = select_config(M, N, K, g_num_sms);
