...
extern "C" void run_mha(
    ...
    // Allocate tile_count_semaphore for DynamicPersistentTileScheduler.
    // The causal/local kernels use this scheduler and atomicAdd on the semaphore
    // to coordinate CTA work distribution. Without it, causal attention crashes
    // with CUDA_ERROR_ILLEGAL_ADDRESS.
    static int *d_tile_count_semaphore = nullptr;
    if (!d_tile_count_semaphore) {
        cudaMalloc(&d_tile_count_semaphore, sizeof(int));
    }
    cudaMemsetAsync(d_tile_count_semaphore, 0, sizeof(int), ext_stream);
    params.tile_count_semaphore = d_tile_count_semaphore;

    run_mha_fwd(params, ext_stream);
}

extern "C" void run_mha_paged(
    void *q_ptr,
    void *k_ptr,
    void *v_ptr,
    void *o_ptr,
    ...
    void *q_norm_weight_ptr,
    float q_norm_eps,
    void *rotary_cos_ptr,
    void *rotary_sin_ptr,
    int32_t *position_ids_ptr,
    int fuse_q_norm_rope,

    cudaStream_t ext_stream
) {
    const uint32_t fuse_num_q_heads = h;
    Flash_fwd_params params;
    memset(&params, 0, sizeof(params));

    // For single-token decode, causal and bidirectional masks are equivalent.
    // Matching the upstream kvcache path here enables its faster/correct GQA
    // layout: view Q/O [B, H, D] as [B, groups, H_k, D] by changing strides.
    if (seqlen_q == 1 && alibi_slopes_ptr == nullptr) {
        is_causal = 0;
    ...
    const bool packed_gqa_decode =
        use_gqa_packing && seqlen_q == 1 && total_q == b;
    const bool seqlenq_ngroups_swapped =
        seqlen_q == 1 && h > h_k && window_size_left < 0 &&
        window_size_right < 0 && d % 8 == 0 &&
        alibi_slopes_ptr == nullptr && !use_gqa_packing;
    if (seqlenq_ngroups_swapped) {
        const uint32_t ngroups = h / h_k;
        q_row_stride = q_head_stride;
        q_head_stride *= ngroups;
        o_row_stride = o_head_stride;
        o_head_stride *= ngroups;
        seqlen_q = ngroups;
    ...
    // Non-packed varlen Q/O ignore batch stride. Packed GQA decode uses it
    // to view Prelude's [batch, heads, dim] Q/O as fixed-length batches.
    params.q_batch_stride = q_batch_stride;
    params.o_batch_stride = o_batch_stride;
    params.alibi_slopes_batch_stride = 0;

    // Paged K/V are [num_blocks, page_block_size, num_heads_k, head_dim].
    params.k_batch_stride = k_batch_stride;
    params.v_batch_stride = v_batch_stride;
    params.q_row_stride = q_row_stride;
    params.k_row_stride = k_row_stride;
    ...
    params.is_bf16 = is_bf16;
    params.cu_seqlens_q =
        (seqlenq_ngroups_swapped || packed_gqa_decode) ? nullptr : cu_seqlens_q_ptr;
    params.cu_seqlens_k =
        (seqlenq_ngroups_swapped || packed_gqa_decode) ? nullptr : cu_seqlens_k_ptr;
    ...
    params.page_num_blocks = (int)page_num_blocks;
    ...
    params.q_norm_weight_ptr = q_norm_weight_ptr;
    params.q_norm_eps = q_norm_eps;
    params.rotary_cos_ptr = rotary_cos_ptr;
    params.rotary_sin_ptr = rotary_sin_ptr;
    params.position_ids_ptr = position_ids_ptr;
    params.fuse_q_norm_rope = fuse_q_norm_rope != 0;
    params.fuse_num_q_heads = (int)fuse_num_q_heads;
