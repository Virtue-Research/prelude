    ...
    typename CollectiveMainloop::Params mainloop_params =
        CollectiveMainloop::to_underlying_arguments({
            ...
            params.page_block_size,
                ...
                : 0,
            static_cast<Element const*>(params.q_norm_weight_ptr),
            params.q_norm_eps,
            static_cast<Element const*>(params.rotary_cos_ptr),
            static_cast<Element const*>(params.rotary_sin_ptr),
            params.position_ids_ptr,
            params.fuse_q_norm_rope,
            params.fuse_num_q_heads
        });
...
void run_mha_fwd_hdim64_gqa(Flash_fwd_params &params, cudaStream_t stream) {
  ...
  BOOL_SWITCH(params.cu_seqlens_q != nullptr, Is_varlen_q, [&] {
    using Seqlen_traits_Q = std::conditional_t<Is_varlen_q, flash::VarGQASeqLenTraits, flash::FixedGQASeqLenTraits>;
    BOOL_SWITCH(params.page_block_size > 0, Is_paged_kv, [&] {
      using Seqlen_traits = std::conditional_t<
          Is_paged_kv,
          flash::PagedSeqLenTraits,
          std::conditional_t<Is_varlen_q, flash::VarSeqLenTraits, flash::FixedSeqLenTraits>>;

      MMA_3WG_SWITCH(kBlockH * params.seqlen_q, kNumMmaWGs, [&] {
        BOOL_SWITCH(params.is_causal, Is_causal, [&] {
          BOOL_SWITCH(params.is_local, Is_local, [&] {
            BOOL_SWITCH(params.num_splits > 1, Is_split_runtime, [&] {
              constexpr bool Is_split = !Seqlen_traits::UseVarSeqLen && Is_split_runtime;
              // BOOL_SWITCH(cutlass::ceil_div(params.seqlen_q, 192/kBlockH) % 2 == 0 && !Is_causal && !Is_local && !Is_split
              //             && kNumMmaWGs == 3, UseCluster, [&] {
                run_flash_fwd<
                  Flash_fwd_kernel_traits<Headdim, kNumMmaWGs * 64, 128, 4 + kNumMmaWGs * 4,
                      2, false, UseCluster ? 2 : 1, T, Is_split, kBlockH>,
                  Is_causal,
                  Is_local && !Is_causal,
                  Seqlen_traits,
                  Seqlen_traits_Q
        ...
        BOOL_SWITCH(params.is_causal, Is_causal, [&] {
          BOOL_SWITCH(params.is_local, Is_local, [&] {
            BOOL_SWITCH(params.num_splits > 1, Is_split_runtime, [&] {
              ...
              //             && kNumMmaWGs == 2, UseCluster, [&] {
                run_flash_fwd<
                  Flash_fwd_kernel_traits<Headdim, kNumMmaWGs * 64, (Is_causal || Is_local || Is_paged_kv) ? 128 : 176,
        ...
        BOOL_SWITCH(params.is_causal, Is_causal, [&] {
          BOOL_SWITCH(params.is_local, Is_local, [&] {
            BOOL_SWITCH(params.num_splits > 1, Is_split_runtime, [&] {
              ...
              //             && kNumMmaWGs == 2, UseCluster, [&] {
                run_flash_fwd<
                  Flash_fwd_kernel_traits<Headdim, kNumMmaWGs * 64, Is_paged_kv ? 64 : (kNumMmaWGs == 1 ? 96 : 80),
                      4 + kNumMmaWGs * 4, 2, false, UseCluster ? 2 : 1, T, Is_split, kBlockH>,
                  Is_causal,
                  Is_local && !Is_causal,
                  Seqlen_traits,
