...
struct CollectiveMainloopFwd {
    ...
    struct Arguments {
        ...
        int page_block_size;
        int num_blocks;
        // Q norm + RoPE prologue (optional)
        Element const* ptr_q_norm_weight;
        float q_norm_eps;
        Element const* ptr_rotary_cos;
        Element const* ptr_rotary_sin;
        int const* position_ids;
        bool fuse_q_norm_rope;
        int num_q_heads;
    };
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.layout_Q, args.layout_K, args.layout_V, args.shape_KV,
                ...
                args.ptr_q_norm_weight, args.q_norm_eps,
                args.ptr_rotary_cos, args.ptr_rotary_sin,
                args.position_ids, args.fuse_q_norm_rope, args.num_q_heads};
    }
    ...
    mma(Params const& mainloop_params,
        ...
        if (mainloop_params.fuse_q_norm_rope) {
            static constexpr int kBlockMN = get<0>(TileShape_MNK{});
            using Prologue = QNormRotaryPrologue<kBlockMN, kHeadDim, NumMmaThreads, kBlockH, Element>;
            Prologue prologue(thread_idx);
            Tensor sQ_pi = cute::as_position_independent_swizzle_tensor(sQ);
            int const cu_seq_start = seqlen_traits_q.cu_seq_len ? seqlen_traits_q.cu_seq_len[bidb] : 0;
            if constexpr (Seqlen_traits_Q::UseGQAPacking) {
                prologue.template apply_fused_norm_rope<true>(
                    sQ_pi, mainloop_params.ptr_q_norm_weight, mainloop_params.q_norm_eps,
                    mainloop_params.ptr_rotary_cos, mainloop_params.ptr_rotary_sin,
                    mainloop_params.position_ids, cu_seq_start, m_block, seqlen_q);
            } else {
                prologue.template apply_fused_norm_rope<false>(
                    sQ_pi, mainloop_params.ptr_q_norm_weight, mainloop_params.q_norm_eps,
                    mainloop_params.ptr_rotary_cos, mainloop_params.ptr_rotary_sin,
                    mainloop_params.position_ids, cu_seq_start, m_block, seqlen_q);
            }
            cutlass::arch::fence_view_async_shared();
            cutlass::arch::NamedBarrier::sync(NumMmaThreads, static_cast<uint32_t>(FwdNamedBarriers::QueryEmpty));
        }
