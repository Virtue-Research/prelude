...
public:
  ...
  static_assert(!(UseVarSeqLen_ && UsePagedKV_ && UseGQAPacking_),
    "Paged variable sequence length with GQA parallelization is not represented by a single traits object.");
  ...
  using StrideT = std::conditional_t<
      ...
      std::conditional_t<
        UseGQAPacking,
        cute::Shape<int64_t, int64_t, _1, int64_t>,
        std::conditional_t<
          !UsePagedKV, 
...
using VarGQASeqLenTraits = SeqLenTraits<true, false, true>;
...
template <>
CUTLASS_DEVICE void VarGQASeqLenTraits::init(int bidb) {
  actual_seq_len =
      seq_used ? seq_used[bidb] : (cu_seq_len[bidb + 1] - cu_seq_len[bidb]);
}
...
// Returns layout of varlen Q/O tensor in packed (M,H/HK,K,HK) format.
template <>
CUTLASS_HOST_DEVICE auto VarGQASeqLenTraits::get_gmem_layout(
    int m, int k, int h_k, int b, int h_h_k_ratio,
    int64_t m_stride, int64_t h_stride, int64_t b_stride, bool padded) const {
  return make_layout(
      make_shape(sum_s + (padded ? kMaxTileSize * b : 0), h_h_k_ratio, k, h_k),
      make_stride(m_stride, h_stride, cute::_1{}, h_stride * h_h_k_ratio));
}

template <>
  CUTLASS_HOST_DEVICE VarGQASeqLenTraits::VirtualShapeT VarGQASeqLenTraits::get_virtual_shape(
      int m, int k, int h_k, int b, int h_h_k_ratio,
      bool padded) const {
    return make_shape(sum_s + (padded ? kMaxTileSize * b : 0), h_h_k_ratio, k, h_k);
  }

template <>
CUTLASS_HOST_DEVICE auto VarGQASeqLenTraits::get_lse_gmem_layout(
    int m, int h, int b, bool padded) const {
  return make_layout(
    make_shape(h, sum_s + (padded ? kMaxTileSize * b : 0)),
    make_stride(int64_t(sum_s + (padded ? kMaxTileSize * b : 0)), cute::_1()));
...
template <>
template <typename MTensor, typename Shape>
CUTLASS_DEVICE auto VarGQASeqLenTraits::get_local_tile_tensor(
    const MTensor &m_tensor, const Shape &tile_shape,
    int bidh_kv, int bidb, bool padded) const {
  auto g_offset = local_tile(
      m_tensor(_, _, _, bidh_kv),
      cute::make_shape(1, get<1>(tile_shape), get<2>(tile_shape)),
      make_coord(cu_seq_len[bidb] + (padded ? kMaxTileSize * bidb : 0), _0{}, _0{}));
  auto g_sequence = make_tensor(
      g_offset.data(),
      make_layout(
        cute::make_shape(actual_seq_len, get<1>(tile_shape), get<2>(tile_shape)),
        g_offset.stride()
      ));
  auto g_tensor = local_tile(g_sequence, tile_shape, make_coord(_, _, _0{}));
  return g_tensor;
}

template <>
template <bool Is_split, typename MTensor, typename Shape>
CUTLASS_DEVICE auto VarGQASeqLenTraits::get_o_local_tile_tensor(
    const MTensor &m_tensor, const Shape &tile_shape,
    int bidh_kv, int bidb, int n_split_idx, bool padded) const {
  static_assert(!Is_split, "Don't currently support split kv kernel with VarGQASeqLenTraits");
  auto g_offset = local_tile(
      m_tensor(_, _, _, bidh_kv),
      cute::make_shape(1, get<1>(tile_shape), get<2>(tile_shape)),
      make_coord(cu_seq_len[bidb] + (padded ? kMaxTileSize * bidb : 0), _0{}, _0{}));
...
template <>
template <bool Is_split, typename MTensor, typename Shape>
CUTLASS_DEVICE auto VarGQASeqLenTraits::get_lse_local_tile_tensor(
    const MTensor &m_tensor, const Shape &tile_shape,
    int bidh, int bidb, int n_split_idx, bool padded) const {
  static_assert(!Is_split, "Don't currently support split kv kernel with VarGQASeqLenTraits");
  auto g_offset = local_tile(
