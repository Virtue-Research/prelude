...
fn main() -> Result<()> {
    // FA3 revival: skip the (slow) CUDA kernel compile to iterate on candle
    // 0.9->0.10 Rust drift fast. The lib (rlib) compiles without the .cu
    // symbols; they resolve only at final binary link. Set FA3_SKIP_KERNELS=1.
    if std::env::var("FA3_SKIP_KERNELS").is_ok() {
        println!("cargo:warning=FA3_SKIP_KERNELS set — skipping kernel compilation");
        println!("cargo:rerun-if-env-changed=FA3_SKIP_KERNELS");
        return Ok(());
    }
    ...
    // flash-attn-v3 targets sm_90a; accept any GPU with compute cap >= 90
    assert!(
        compute_cap >= 90,
        "Compute capability must be >= 90 (sm_90a), got {compute_cap}"
    );
    ...
    if should_compile {
        ...
        cu_files
            ...
            .try_for_each(|(input, obj)| -> Result<()> {
                ...
                // Fix CUTLASS compatibility with CUDA 13.x (PFN_cuTensorMapEncodeTiled undefined)
                command.arg("-DCUTLASS_ENABLE_DIRECT_CUDA_DRIVER_CALL");
