...
impl FlashAttnVarLen {
    ...
    >(
        ...
        let requested_gqa_packing = match num_heads / num_heads_k {
            2 | 4 | 8 | 16 | 32 => self.use_gqa_packing,
            _ => false,
        };
        let use_gqa_packing = requested_gqa_packing as i32;
...
struct FlashAttnVarLenPaged {
    ...
    pub window_size_right: Option<usize>,
    pub use_gqa_packing: bool,
    /// When set, apply Q RMSNorm + RoPE in the FA3 SMEM prologue (Plan A).
    pub q_prologue: Option<QPrologueParams>,
}

pub struct QPrologueParams {
...
impl FlashAttnVarLenPaged {
    ...
    >(
        ...
        let (block_table, block_table_layout) = self.block_table.storage_and_layout();
        let block_table = match &*block_table {
            candle::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?,
            _ => candle::bail!("block_table must be a cuda tensor"),
        };
        let block_table = match block_table_layout.contiguous_offsets() {
            Some((o1, o2)) => block_table.slice(o1..o2),
            None => candle::bail!("block_table has to be contiguous"),
        };
        ...
        if q_stride.len() != 3 || k_stride.len() != 4 || v_stride.len() != 4 {
            candle::bail!(
                "flash-attn-v3-varlen-paged expects q rank 3 and k/v rank 4 (q: {:?}, k: {:?}, v: {:?})",
                q_l.shape(),
                k_l.shape(),
                v_l.shape()
            );
        }
        if q_stride[2] != 1 || k_stride[3] != 1 || v_stride[3] != 1 {
            candle::bail!("q/k/v last dimension must be contiguous");
        }

        let (total_q, num_heads, head_size_og) = q_l.shape().dims3()?;
        let (num_blocks_k, page_block_size_k, num_heads_k, _head_size_k) = k_l.shape().dims4()?;
        let (num_blocks_v, page_block_size_v, num_heads_v, head_size_v) = v_l.shape().dims4()?;
        if num_blocks_k != num_blocks_v
            || page_block_size_k != page_block_size_v
            || num_heads_k != num_heads_v
            || head_size_og != head_size_v
        {
            candle::bail!(
                "k/v paged cache shape mismatch: k={:?}, v={:?}",
                k_l.shape(),
                v_l.shape()
            );
        }
        let page_block_size = page_block_size_k;
        if head_size_og > 256 {
            candle::bail!("only supports head dimension at most 256 (got {head_size_og})")
        }
        if !(head_size_og == 256 || head_size_og == 128 || head_size_og == 64) {
        ...
        let nseqlens_q = seqlens_q_layout.shape().dims1()?;
        let nseqlens_k = seqlens_k_layout.shape().dims1()?;
        if nseqlens_q < 2 || nseqlens_k != nseqlens_q {
            candle::bail!(
                "seqlens_q/seqlens_k must have same len >=2, got {nseqlens_q}/{nseqlens_k}"
            );
        }
        let batch_size = nseqlens_q - 1;
        let decode_packed_q = self.max_seqlen_q == 1 && total_q == batch_size;
        let fixed_paged_q = !decode_packed_q && total_q == batch_size * self.max_seqlen_q;
        let use_gqa_packing = if requested_gqa_packing && (decode_packed_q || fixed_paged_q) {
            1
        } else {
            0
        };
        let (bt_batch, _bt_cols) = block_table_layout.shape().dims2()?;
        if bt_batch != batch_size {
            candle::bail!("block_table batch dim mismatch: expected {batch_size}, got {bt_batch}");
        }
        ...
        unsafe {
            ...
            let (seqlens_q_ptr, _guard) = seqlens_q.device_ptr(&stream);
            let (seqlens_k_ptr, _guard) = seqlens_k.device_ptr(&stream);
            let (block_table_ptr, _guard) = block_table.device_ptr(&stream);
            let raw_stream = stream.cu_stream() as *mut core::ffi::c_void;
            let cu_seqlens_q_for_ffi = if fixed_paged_q {
                std::ptr::null::<i32>()
            } else {
                seqlens_q_ptr as *const i32
            };
            let q_batch_stride = if fixed_paged_q {
                q_stride[0] * self.max_seqlen_q
            } else {
                q_stride[0]
            };
            let o_batch_stride = if fixed_paged_q {
                o_stride[0] * self.max_seqlen_q
            } else {
                o_stride[0]
            };

            let mut q_norm_weight_ptr = std::ptr::null::<core::ffi::c_void>();
            let mut q_norm_eps = 0.0f32;
            let mut rotary_cos_ptr = std::ptr::null::<core::ffi::c_void>();
            let mut rotary_sin_ptr = std::ptr::null::<core::ffi::c_void>();
            let mut position_ids_ptr = std::ptr::null::<i32>();
            let mut fuse_q_norm_rope = 0i32;
            if let Some(prologue) = &self.q_prologue {
                let (qw, qw_l) = prologue.q_norm_weight.storage_and_layout();
                let qw = match &*qw {
                    candle::Storage::Cuda(c) => c.as_cuda_slice::<T>()?,
                    _ => candle::bail!("q_norm_weight must be a cuda tensor"),
                };
                let qw = qw.slice(qw_l.start_offset()..);
                let (qw_ptr, _qw_guard) = qw.device_ptr(&stream);
                q_norm_weight_ptr = qw_ptr as *const core::ffi::c_void;
                q_norm_eps = prologue.eps;

                let (cos, cos_l) = prologue.cos.storage_and_layout();
                let cos = match &*cos {
                    candle::Storage::Cuda(c) => c.as_cuda_slice::<T>()?,
                    _ => candle::bail!("rotary cos must be a cuda tensor"),
                };
                let cos = cos.slice(cos_l.start_offset()..);
                let (cos_ptr, _cos_guard) = cos.device_ptr(&stream);
                rotary_cos_ptr = cos_ptr as *const core::ffi::c_void;

                let (sin, sin_l) = prologue.sin.storage_and_layout();
                let sin = match &*sin {
                    candle::Storage::Cuda(c) => c.as_cuda_slice::<T>()?,
                    _ => candle::bail!("rotary sin must be a cuda tensor"),
                };
                let sin = sin.slice(sin_l.start_offset()..);
                let (sin_ptr, _sin_guard) = sin.device_ptr(&stream);
                rotary_sin_ptr = sin_ptr as *const core::ffi::c_void;

                let (pos, pos_l) = prologue.position_ids.storage_and_layout();
                let pos = match &*pos {
                    candle::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?,
                    _ => candle::bail!("position_ids must be a cuda tensor"),
                };
                let pos = match pos_l.contiguous_offsets() {
                    Some((o1, o2)) => pos.slice(o1..o2),
                    None => candle::bail!("position_ids has to be contiguous"),
                };
                let (pos_ptr, _pos_guard) = pos.device_ptr(&stream);
                position_ids_ptr = pos_ptr as *const i32;
                fuse_q_norm_rope = 1;
            }

            macro_rules! run_mha_paged {
                ($seqused_k_ptr:expr) => {
                    ffi::run_mha_paged(
                        q_ptr as *const core::ffi::c_void,
                        k_ptr as *const core::ffi::c_void,
                        ...
                        o_stride[0] as u32,
                        o_stride[1] as u32,
                        o_batch_stride as u32,
                        block_table_layout.stride()[0] as u32,
                        batch_size as u32,
                        batch_size as u32,
                        num_heads as u32,
                        num_heads_k as u32,
            ...
            if let Some(seqused_k) = &self.seqused_k {
                let (seqused_k, seqused_k_layout) = seqused_k.storage_and_layout();
                let nseqused_k = seqused_k_layout.shape().dims1()?;
                if nseqused_k != batch_size {
                    candle::bail!(
                        "seqused_k batch dim mismatch: expected {batch_size}, got {nseqused_k}"
                    );
                }
                let seqused_k = match &*seqused_k {
                    candle::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?,
                    _ => candle::bail!("seqused_k must be a cuda tensor"),
                };
                let seqused_k = match seqused_k_layout.contiguous_offsets() {
                    Some((o1, o2)) => seqused_k.slice(o1..o2),
                    None => candle::bail!("seqused_k has to be contiguous"),
                };
                let (seqused_k_ptr, _seqused_k_guard) = seqused_k.device_ptr(&stream);
                run_mha_paged!(seqused_k_ptr as *const i32)
            } else {
                run_mha_paged!(std::ptr::null::<i32>())
            }
        }
...
impl candle::CustomOp3 for FlashAttnVarLenPaged {
    fn name(&self) -> &'static str {
        "flash-attn-v3-varlen-paged"
    }
    ...
    fn cpu_fwd(
        ...
        candle::bail!("no cpu support for flash-attn-v3-varlen-paged")
    }
...
pub fn flash_attn_varlen_paged(
    ...
    flash_attn_varlen_paged_with_seqused_k(
        q,
        k_paged,
        v_paged,
        block_table,
...
#[allow(clippy::too_many_arguments)]
pub fn flash_attn_varlen_paged_with_seqused_k(
    q: &Tensor,
    k_paged: &Tensor,
    v_paged: &Tensor,
    block_table: &Tensor,
    ...
    flash_attn_varlen_paged_with_seqused_k_and_prologue(
        q,
        k_paged,
        v_paged,
        block_table,
...
#[allow(clippy::too_many_arguments)]
pub fn flash_attn_varlen_paged_with_seqused_k_and_prologue(
    q: &Tensor,
    k_paged: &Tensor,
    v_paged: &Tensor,
    block_table: &Tensor,
    ...
    causal: bool,
    use_gqa_packing: bool,
    q_prologue: Option<QPrologueParams>,
) -> Result<Tensor> {
    let window_size_left = None;
    let window_size_right = if causal { Some(0) } else { None };

    let op = FlashAttnVarLenPaged {
        softmax_scale,
        max_seqlen_q,
        max_seqlen_k,
        seqlens_q: seqlens_q.clone(),
    ...
    q.apply_op3(k_paged, v_paged, op)
}
