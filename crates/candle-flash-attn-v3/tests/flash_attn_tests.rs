...
fn reference_decode_gqa(
    ...
    let mut out = vec![0f32; k_lens.len() * num_q_heads * head_dim];
    let mut k_base = 0usize;
    for (b, &sk) in k_lens.iter().enumerate() {
        for hq in 0..num_q_heads {
            let hkv = hq / groups;
            let q_off = b * num_q_heads * head_dim + hq * head_dim;
            let mut scores = vec![0f32; sk];
            for kj in 0..sk {
                let k_off = (k_base + kj) * num_kv_heads * head_dim + hkv * head_dim;
                let mut dot = 0f32;
                for d in 0..head_dim {
                    dot += q[q_off + d] * k[k_off + d];
                }
                scores[kj] = dot * softmax_scale;
            }
            ...
            let out_off = b * num_q_heads * head_dim + hq * head_dim;
            for kj in 0..sk {
                let weight = scores[kj] / denom;
                let v_off = (k_base + kj) * num_kv_heads * head_dim + hkv * head_dim;
                for d in 0..head_dim {
...
#[test]
fn flash_attn_varlen_paged_gqa_decode_matches_reference() -> Result<()> {
    let device = Device::new_cuda(0)?;
    let k_lens = [513usize, 300];
    let batch = k_lens.len();
    let num_q_heads = 32usize;
    let num_kv_heads = 4usize;
    let head_dim = 128usize;
    let page_size = 256usize;
    let num_blocks = 6usize;
    let max_blocks = 3usize;
    let total_k = k_lens.iter().sum::<usize>();
    let softmax_scale = 1.0f32 / (head_dim as f32).sqrt();

    let q_data: Vec<f32> = (0..batch * num_q_heads * head_dim)
        .map(|i| (((i * 13) % 97) as f32 - 48.0) / 200.0)
        .collect();
    let k_data: Vec<f32> = (0..total_k * num_kv_heads * head_dim)
        .map(|i| (((i * 17 + 7) % 89) as f32 - 44.0) / 190.0)
        .collect();
    let v_data: Vec<f32> = (0..total_k * num_kv_heads * head_dim)
        .map(|i| (((i * 19 + 3) % 83) as f32 - 41.0) / 160.0)
        .collect();

    let block_tables = [[2u32, 0u32, 4u32], [1u32, 3u32, 5u32]];
    let mut k_cache = vec![4.0f32; num_blocks * page_size * num_kv_heads * head_dim];
    let mut v_cache = vec![-3.0f32; num_blocks * page_size * num_kv_heads * head_dim];
    let mut k_base = 0usize;
    for (seq_idx, &sk) in k_lens.iter().enumerate() {
        for pos in 0..sk {
            let block = block_tables[seq_idx][pos / page_size] as usize;
            let offset = pos % page_size;
            for h in 0..num_kv_heads {
                let src = (k_base + pos) * num_kv_heads * head_dim + h * head_dim;
                let dst = ((block * page_size + offset) * num_kv_heads + h) * head_dim;
                k_cache[dst..dst + head_dim].copy_from_slice(&k_data[src..src + head_dim]);
                v_cache[dst..dst + head_dim].copy_from_slice(&v_data[src..src + head_dim]);
            }
        }
        k_base += sk;
    }

    let q = Tensor::from_vec(q_data.clone(), (batch, num_q_heads, head_dim), &device)?
        .to_dtype(DType::BF16)?;
    ...
    .to_dtype(DType::BF16)?;
    let block_table_flat: Vec<u32> = block_tables.into_iter().flatten().collect();
    let block_table = Tensor::from_vec(block_table_flat, (batch, max_blocks), &device)?;
    let cu_q = Tensor::from_vec(vec![0u32, 1, 2], (batch + 1,), &device)?;
    let cu_k = Tensor::from_vec(vec![0u32, 513, 813], (batch + 1,), &device)?;
    let seqused_k = Tensor::from_vec(vec![513u32, 300], (batch,), &device)?;

    let got = candle_flash_attn_v3::flash_attn_varlen_paged_with_seqused_k(
        &q,
        &k_cache,
        &v_cache,
        &block_table,
    ...
    .flatten_all()?
    .to_vec1::<f32>()?;
    let expected = reference_decode_gqa(
        &q_data,
        &k_data,
        &v_data,
        &k_lens,
    ...
    let max_diff = got
        ...
        .map(|(a, b)| (a - b).abs())
        .fold(0.0f32, f32::max);
    assert!(max_diff < 3e-2, "max_diff={max_diff}");
    Ok(())
}

#[test]
fn flash_attn_varlen_paged_gqa_decode_long_context_matches_reference() -> Result<()> {
    let device = Device::new_cuda(0)?;
    let k_lens = [2482usize];
    let batch = k_lens.len();
    let num_q_heads = 32usize;
    let num_kv_heads = 4usize;
    let head_dim = 128usize;
    let page_size = 256usize;
    let max_blocks = k_lens[0].div_ceil(page_size);
    let num_blocks = 12usize;
    let total_k = k_lens.iter().sum::<usize>();
    let softmax_scale = 1.0f32 / (head_dim as f32).sqrt();
    ...
    let block_tables = [[7u32, 2, 9, 0, 5, 11, 1, 4, 8, 3]];
    let mut k_cache = vec![4.0f32; num_blocks * page_size * num_kv_heads * head_dim];
    let mut v_cache = vec![-3.0f32; num_blocks * page_size * num_kv_heads * head_dim];
    let mut k_base = 0usize;
    for (seq_idx, &sk) in k_lens.iter().enumerate() {
    ...
    let block_table_flat: Vec<u32> = block_tables.into_iter().flatten().collect();
    let block_table = Tensor::from_vec(block_table_flat, (batch, max_blocks), &device)?;
    let cu_q = Tensor::from_vec(vec![0u32, 1], (batch + 1,), &device)?;
    let cu_k = Tensor::from_vec(vec![0u32, k_lens[0] as u32], (batch + 1,), &device)?;
    let seqused_k = Tensor::from_vec(vec![k_lens[0] as u32], (batch,), &device)?;
...
fn reference_prefill_last_gqa(
    ...
    for hq in 0..num_q_heads {
        let hkv = hq / groups;
        let q_off = qi * num_q_heads * head_dim + hq * head_dim;
        let mut scores = vec![0f32; seq_len];
        for kj in 0..seq_len {
            let k_off = kj * num_kv_heads * head_dim + hkv * head_dim;
            let mut dot = 0f32;
            for d in 0..head_dim {
                dot += q[q_off + d] * k[k_off + d];
            }
        ...
        for kj in 0..seq_len {
            let weight = scores[kj] / denom;
            let v_off = kj * num_kv_heads * head_dim + hkv * head_dim;
            for d in 0..head_dim {
                out[out_off + d] += weight * v[v_off + d];
            }
        }
...
#[test]
fn flash_attn_varlen_paged_gqa_prefill_long_context_last_token_matches_reference() -> Result<()> {
    let device = Device::new_cuda(0)?;
    let seq_len = 2481usize;
    let batch = 1usize;
    let num_q_heads = 32usize;
    let num_kv_heads = 4usize;
    let head_dim = 128usize;
    let page_size = 256usize;
    let max_blocks = seq_len.div_ceil(page_size);
    let num_blocks = 12usize;
    let softmax_scale = 1.0f32 / (head_dim as f32).sqrt();

    let q_data: Vec<f32> = (0..seq_len * num_q_heads * head_dim)
        .map(|i| (((i * 13 + 5) % 97) as f32 - 48.0) / 200.0)
        .collect();
    ...
    for pos in 0..seq_len {
        let block = block_tables[0][pos / page_size] as usize;
        let offset = pos % page_size;
        for h in 0..num_kv_heads {
            let src = pos * num_kv_heads * head_dim + h * head_dim;
            let dst = ((block * page_size + offset) * num_kv_heads + h) * head_dim;
            k_cache[dst..dst + head_dim].copy_from_slice(&k_data[src..src + head_dim]);
            v_cache[dst..dst + head_dim].copy_from_slice(&v_data[src..src + head_dim]);
        }
    ...
    let q = Tensor::from_vec(q_data.clone(), (seq_len, num_q_heads, head_dim), &device)?
        .to_dtype(DType::BF16)?;
    let k_cache = Tensor::from_vec(
        k_cache,
        (num_blocks, page_size, num_kv_heads, head_dim),
    ...
    let block_table_flat: Vec<u32> = block_tables.into_iter().flatten().collect();
    let block_table = Tensor::from_vec(block_table_flat, (batch, max_blocks), &device)?;
    let cu_q = Tensor::from_vec(vec![0u32, seq_len as u32], (batch + 1,), &device)?;
    let cu_k = Tensor::from_vec(vec![0u32, seq_len as u32], (batch + 1,), &device)?;
    let seqused_k = Tensor::from_vec(vec![seq_len as u32], (batch,), &device)?;
    ...
    .flatten_all()?
    .to_vec1::<f32>()?;
    let expected = reference_prefill_last_gqa(
        &q_data,
        &k_data,
        &v_data,
        seq_len,
...
fn reference_varlen_causal_gqa(
    q: &[f32],
    k: &[f32],
    v: &[f32],
    q_lens: &[usize],
    ...
    let groups = num_q_heads / num_kv_heads;
    let total_q = q_lens.iter().sum::<usize>();
    let mut out = vec![0f32; total_q * num_q_heads * head_dim];
    let mut q_base = 0usize;
    let mut k_base = 0usize;
    for (&sq, &sk) in q_lens.iter().zip(k_lens.iter()) {
        let offset = sk - sq;
        for qi in 0..sq {
            let allowed = offset + qi + 1;
            for hq in 0..num_q_heads {
                let hkv = hq / groups;
                let q_off = (q_base + qi) * num_q_heads * head_dim + hq * head_dim;
                let mut scores = vec![0f32; allowed];
                for kj in 0..allowed {
                    let k_off = (k_base + kj) * num_kv_heads * head_dim + hkv * head_dim;
                    let mut dot = 0f32;
                    for d in 0..head_dim {
                ...
                let out_off = (q_base + qi) * num_q_heads * head_dim + hq * head_dim;
                for kj in 0..allowed {
                    let weight = scores[kj] / denom;
                    let v_off = (k_base + kj) * num_kv_heads * head_dim + hkv * head_dim;
                    for d in 0..head_dim {
...
#[test]
fn flash_attn_varlen_gqa_unequal_lengths_matches_reference() -> Result<()> {
    let device = Device::new_cuda(0)?;
    let q_lens = [7usize, 3, 5];
    let k_lens = [7usize, 3, 5];
    ...
    let q_data: Vec<f32> = (0..total_q * num_q_heads * head_dim)
        .map(|i| (((i * 11 + 5) % 101) as f32 - 50.0) / 210.0)
        .collect();
    let k_data: Vec<f32> = (0..total_k * num_kv_heads * head_dim)
        .map(|i| (((i * 17 + 9) % 97) as f32 - 48.0) / 220.0)
        .collect();
    let v_data: Vec<f32> = (0..total_k * num_kv_heads * head_dim)
        .map(|i| (((i * 19 + 3) % 89) as f32 - 44.0) / 180.0)
        .collect();

    let q = Tensor::from_vec(q_data.clone(), (total_q, num_q_heads, head_dim), &device)?
        .to_dtype(DType::BF16)?;
    let k = Tensor::from_vec(k_data.clone(), (total_k, num_kv_heads, head_dim), &device)?
        .to_dtype(DType::BF16)?;
    let v = Tensor::from_vec(v_data.clone(), (total_k, num_kv_heads, head_dim), &device)?
        .to_dtype(DType::BF16)?;
    let cu_q = Tensor::from_vec(vec![0u32, 7, 10, 15], (batch + 1,), &device)?;
    let cu_k = Tensor::from_vec(vec![0u32, 7, 10, 15], (batch + 1,), &device)?;

    let got = candle_flash_attn_v3::flash_attn_varlen(
        &q,
        &k,
        &v,
        &cu_q,
        &cu_k,
        *q_lens.iter().max().unwrap(),
        *k_lens.iter().max().unwrap(),
        softmax_scale,
        true,
        true,
    ...
    .flatten_all()?
    .to_vec1::<f32>()?;
    let expected = reference_varlen_causal_gqa(
        &q_data,
        &k_data,
        &v_data,
        &q_lens,
...
#[test]
fn flash_attn_varlen_paged_gqa_prefill_matches_reference() -> Result<()> {
    let device = Device::new_cuda(0)?;
    let q_lens = [5usize, 3];
    let k_lens = [513usize, 300];
    let batch = k_lens.len();
    ...
    let block_table_flat: Vec<u32> = block_tables.into_iter().flatten().collect();
    let block_table = Tensor::from_vec(block_table_flat, (batch, max_blocks), &device)?;
    let cu_q = Tensor::from_vec(vec![0u32, 5, 8], (batch + 1,), &device)?;
    let cu_k = Tensor::from_vec(vec![0u32, 513, 813], (batch + 1,), &device)?;
    let seqused_k = Tensor::from_vec(vec![513u32, 300], (batch,), &device)?;
