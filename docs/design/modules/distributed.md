...
```rust
// prelude-core/src/modules/linear.rs — Linear::forward internals for TP
...
impl Linear {
    fn forward(&self, x: &Tensor, ctx: &BatchState, ops: &Ops) -> Result<Tensor> {
        ...
        //
        let out = ops.gemm.matmul(x, &self.weight)?;  // self.weight is already the shard

        // Step 2: LoRA (if configured) — also local, no communication

        // Step 3: TP communication
        match self.tp {
            TpMode::Row => ops.comm.all_reduce_sum(&out),
            //   Each rank has a partial sum → all_reduce produces full output on all ranks.
            TpMode::Column { gather_output: true } => ops.comm.all_gather(&out, -1),
...
AFD is handled by `modules::moe_layer` as a third distribution mode alongside local and EP:
...
**Model code is unchanged.** The model still calls `modules::moe_layer(&h, &gate, &weights, &config, ops)`.
