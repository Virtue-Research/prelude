#!/usr/bin/env python3
"""Accuracy test for Qwen3.5 dense hybrid model (0.8B, 2B, 4B, 9B, 27B).
...
Usage:
    ...
    python tests/qwen3_5/test_accuracy.py --generate-golden \
        --model /path/to/models/qwen3.5-0.8b

    # Step 2: Test against pre-started server
    python tests/qwen3_5/test_accuracy.py --server-url http://localhost:8001

    # Step 2 (alt): Auto-start server and test
    python tests/qwen3_5/test_accuracy.py \
        --model /path/to/models/qwen3.5-0.8b
...
def start_server(binary: str, model: str, port: int) -> subprocess.Popen:
    """Start prelude-server with Qwen3.5 model."""
    env = os.environ.copy()
