#!/usr/bin/env python3
"""Accuracy test for Qwen3-Next hybrid model (80B total / 3B active).
...
Usage:
    # Step 1: Generate golden reference (one-time, ~200s to load model)
    python tests/qwen3_next/test_accuracy.py --generate-golden \
        --model /path/to/models/qwen3-next

    # Step 2: Test against pre-started server
    python tests/qwen3_next/test_accuracy.py --server-url http://localhost:8001

    # Step 2 (alt): Auto-start server and test
    python tests/qwen3_next/test_accuracy.py \
        --model /path/to/models/qwen3-next
...
DEFAULT_PORT = 8099
HEALTH_TIMEOUT = 600  # 10 min — model is ~160GB, slow to load
TOP_K_LOGPROBS = 5
...
def generate_golden(model_path: str, prompts: list[dict], output_path: Path,
    ...
    print("(This will take ~3 minutes for 80B model)")
    t0 = time.time()
...
def start_server(binary: str, model: str, port: int) -> subprocess.Popen:
    """Start prelude-server with Qwen3-Next model."""
    env = os.environ.copy()
...
def compare(ref: dict, response: dict, spec: dict, use_logprobs: bool) -> dict:
    ...
    Returns dict with pass/fail status and diagnostic info.
    """
...
def main():
    parser = argparse.ArgumentParser(description="Qwen3-Next accuracy test")
    ...
    golden_path = Path(args.golden_path) if args.golden_path else SCRIPT_DIR / "golden" / f"qwen3-next-{dtype_label}.json"
